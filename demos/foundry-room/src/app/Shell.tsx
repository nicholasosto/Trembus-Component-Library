...
/**
 * The app shell: a sticky header (brand + Rooms/Laws nav + theme toggle), the
 * routed `<Outlet />`, and a footer. Composed from @trembus/ui primitives; the
 * outer `.tcl-root` makes the tree inherit the themed font/bg/text.
 */
    ...
    <div className="tcl-root app">
      ...
      <Box as="header" surface="raised" border="soft" px={6} py={4} className="app__header">
        <Inline align="center" justify="between" gap={5} wrap>
          <Inline align="baseline" gap={3}>
            <Text as="span" size="lg" className="app__brand">
              Foundry
            </Text>
          ...
          <NavBar aria-label="Primary">
            ...
            <NavBar.Link asChild>
              <a aria-disabled href="#archive" onClick={(e) => e.preventDefault()}>
                Archive
              </a>
            </NavBar.Link>
          </NavBar>
      ...
      <Box as="footer" surface="sunken" border="soft" px={6} py={5}>
        <Text size="sm" tone="dim">
          ...
          — FolderTree, Dialog, Table, Badge, Tooltip, and the new{' '}
          <Text as="span" size="sm" mono>
            Stepper
          </Text>{' '}
