...
conventions (see AGENTS.md) and wires it into that package's `src/index.ts`. The generated
...
```bash
node .Codex/skills/new-component/scaffold.mjs <Name> [--pkg ui|viz] [--lead reveal-state|afford-action|acknowledge-input]
```
