...
| [`@trembus/viz`](packages/viz#readme)           | [![npm](https://img.shields.io/npm/v/@trembus/viz.svg)](https://www.npmjs.com/package/@trembus/viz)           | Tier-2 node-link visualizations — `Tree`, `Lineage`, `SystemMap`, `ClassDiagram`, `Strata` (hierarchy · DAG · C4 drill-down · UML · first-principles strata). Tokens only — never depends on `@trembus/ui`.                      |
