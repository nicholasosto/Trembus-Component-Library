...
| [`@trembus/icons`](packages/icons#readme)       | [![npm](https://img.shields.io/npm/v/@trembus/icons.svg)](https://www.npmjs.com/package/@trembus/icons)       | The shared glyph set — 50 tree-shakeable inline-SVG icons (node-kind / file-type marks, the workflow-output vocabulary for human+AI command centers, UI affordances) + kind/provenance/filename maps, consumed by `@trembus/ui`, `@trembus/viz`, and `@trembus/game-viz`. React-only — no token dep, no stylesheet. |
| [`@trembus/ui`](packages/ui#readme)             | [![npm](https://img.shields.io/npm/v/@trembus/ui.svg)](https://www.npmjs.com/package/@trembus/ui)             | The component library — primitives (`Box` / `Stack` / `Text` / `Pressable`), form controls, overlays, command chrome (`Toolbar`, `Menu`, `CommandBar`), and Tier-1 data viz (charts, `Heatmap`, `DecisionMap`, …). Depends on `@trembus/tokens`.                                                                    |
| [`@trembus/viz`](packages/viz#readme)           | [![npm](https://img.shields.io/npm/v/@trembus/viz.svg)](https://www.npmjs.com/package/@trembus/viz)           | Tier-2 node-link visualizations — `Tree`, `Lineage`, `SystemMap`, `ClassDiagram`, `Strata`, `TalentTree`, `Nebula` (hierarchy · DAG · C4 drill-down · UML · first-principles strata · game skill-tree · 3D concept map by relatedness). Tokens only — never depends on `@trembus/ui`.                                                                         |
| [`@trembus/game-viz`](packages/game-viz#readme) | [![npm](https://img.shields.io/npm/v/@trembus/game-viz.svg)](https://www.npmjs.com/package/@trembus/game-viz) | Expressive game / cinematic UI — HUD frames, character dossiers, episode decks, title plates, 3D model thumbnails, a gothic talent-tree star-chart. A liturgical-gothic idiom over the same contract: _theatrical surface, accessible spine_.                                                                       |
